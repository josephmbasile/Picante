...
#Section 2A: Stack overflow functions.
...
#area of polygon poly from stackoverflow
def find_polygon_area_2(poly):
    """Returns the area of a polygon, including for convex shapes.
...
def add_extra_points_to_segment(line,max_size):
    """Divides a line segment into smaller line segments based on the max size.
    
    Line should be formatted as a list of two points.

    Points should be formatted as a list of coordinates X, Y, Z.

    Returns a list of points with the original endpoints included.
    """
    vector = get_vector_from_line_segment(line)
    magnitude = get_magnitude_of_vector(vector)
    #print(f"magnitude: {magnitude}; max_size: {max_size}")
    num_points = int(np.floor(magnitude/max_size))
    if num_points < 0:
        num_points = 0
    new_magntiude = magnitude/(num_points+1)
    new_points = [line[0]]
    for i in range(num_points):
        new_vector = scale_vector_to_magnitude(vector, new_magntiude*(i+1))
        new_point = add_two_vectors_decimal(line[0],new_vector)
        new_points.append(new_point)
    new_points.append(line[1])
    return new_points
...
def are_these_line_segments_the_same(line, check_line):
    """Checks if two line segments are the same, within a tolerance.
    
    The line segments should be formatted as a list of two points.

    The points should be formatted as a list of coordinates X, Y, Z.

    Returns true if the line segments are the same, including if the points are reversed.
    """
    tolerance = 1E-6
    line_length = find_line_length(line)
    check_line_length = find_line_length(check_line)
    if line_length > 0 and np.abs(check_line_length - line_length) < tolerance:
        points_match = []
        for i in range(len(line)):
            point_match = False
            for k in range(len(check_line)):
                if np.abs(line[i][0] - check_line[k][0]) < tolerance and np.abs(line[i][1] - check_line[k][1]) < tolerance and np.abs(line[i][2] - check_line[k][2]) < tolerance:
                    point_match = True
            points_match.append(point_match)
        if points_match[0] and points_match[1]:
            return True
        else:
            return False
    elif line_length <tolerance or check_line_length <tolerance:
        #print(f"Zero length line detected.")
        return False
    else:
        return False

def combine_adjacent_coplanar_triangles(polyhedron):
    """Combines polyhedron triangles that are coplanar and coincident.
    
    Polyhedron should be formatted as a list of triangles.

    Triangles should be formatted as a list of points.

    Points should be formatted as a list of coordinates X, Y, Z.

    Returns a polyhedron consisting of the minimum number of polygons.
    """
    working_polyhedron = [
        polygon
    ...
    combined_polyhedron = []
    for i in range(len(working_polyhedron)):
        if working_polyhedron[i] != []:
            combined_polyhedron.append(working_polyhedron[i])
            working_polyhedron[i] = []
            no_match = False
            while no_match == False:
                matches = 0
                
                for j in range(len(working_polyhedron)):
                    if working_polyhedron[j] != []:
                        #print(f"len-working triangle: {len(working_polyhedron[j])}")
                        point_matches = []
                        for r in range(len(combined_polyhedron[-1])):
                            point_check = combined_polyhedron[-1][r]
                            point_1_match = are_these_points_the_same(working_polyhedron[j][0],point_check)
                            point_2_match = are_these_points_the_same(working_polyhedron[j][1],point_check)
                            point_3_match = are_these_points_the_same(working_polyhedron[j][2],point_check)
                            if point_1_match or point_2_match or point_3_match:
                                point_matches.append(point_check)
                        
                        
                        if len(point_matches) == 2:
                            #print(f"Point matches: {point_matches}")
                            index_1 = find_point_index(combined_polyhedron[-1],point_matches[0])
                            index_2 = find_point_index(combined_polyhedron[-1],point_matches[1])
                            index_list = [index_1,index_2]
                            index_list.sort()
                            insert_point = []
                            dont_insert_points = []
                            for u in range(len(working_polyhedron[j])):
                                this_index = find_point_index(combined_polyhedron[-1],working_polyhedron[j][u])
                                if this_index is False:
                                    insert_point.append(working_polyhedron[j][u])
                                else:
                                    dont_insert_points.append(working_polyhedron[j][u])
                            #print(f"Insert point: {insert_point}")

                            coplanar_check = [
                                point
                                for point in combined_polyhedron[-1]
                            ]
                            insert_index = index_list[-1]
                            if index_list[0] == 0 and index_list[1] != 1:
                                insert_index = 0
                            coplanar_check.insert(insert_index,insert_point[0])
                            coplanar_check = remove_duplicate_points(coplanar_check)
                            is_coplanar = check_if_polygon_is_coplanar(coplanar_check)
                            #print(f"is_coplanar: {is_coplanar}")
                            if is_coplanar == True:
                                #print(f"Inserting point {insert_point} into index {index_list[-1]} of polygon {combined_polyhedron[-1]}")
                                #print(f"Not inserting matching points: {dont_insert_points}")
                                matches = matches + 1
                                combined_polyhedron[-1] = coplanar_check
                                working_polyhedron[j] = []
                                #print(f"Match Found!!!!!!!!!!!!!!!")
                if matches == 0: 
                    no_match = True
                    #print(f"no matches: {len(combined_polyhedron)}")
    for m in range(len(combined_polyhedron)):
        reduced_face = remove_duplicate_points(combined_polyhedron[m])
        combined_polyhedron[m] = reduced_face
    #print(f"len combined_polyhedron: {len(combined_polyhedron)}")
    return combined_polyhedron


def find_point_index(polygon,point):
    """Finds the index of the specified point in the specified polygon.
    ...
    Returns False if the point is not found in the polygon."""

    for i in range(len(polygon)):
        is_point = are_these_points_the_same(polygon[i],point)
        if is_point == True:
            return i
    return False
    
def are_these_points_the_same(point_1, point_2):
    """Checks if two points are the same to within a tolerance."""
    tolerance = 1E-8

    if np.abs(point_1[0] - point_2[0]) < tolerance and np.abs(point_1[1] - point_2[1]) < tolerance and np.abs(point_1[2] - point_2[2]) < tolerance:
        return True
    else:
        return False
...
def find_centroid_of_polygon(planar_polygon):
    ...
    for triangle in triangles:
        ...
        if type(centroid) == str:
            print(centroid)
            print(f"planar_polygon: {planar_polygon}")
            print(f"triangle: {triangle}")
            #print(f"centroid[0]: {centroid[0]}; area: {area}")
...
def find_approximate_centroid_of_polygon(planar_polygon):
    ...
    if abs(plane_equation[0][0]) > tolerance:
    ...
    elif abs(plane_equation[0][1]) > tolerance:
    ...
    elif abs(plane_equation[0][2]) > tolerance:
...
def get_D_for_sphere(x, y, a, b, c):
    return a[x] * (b[y] - c[y]) + b[x] * (c[y] - a[y]) + c[x] * (a[y] - b[y])

def get_E_for_sphere(x, y, a, b, c, d, ra, rb, rc, rd, uvw):
    return ( ra * get_D_for_sphere(x, y, b, c, d) - rb * get_D_for_sphere(x, y, c, d, a) + rc * get_D_for_sphere(x, y, d, a, b) - rd * get_D_for_sphere(x, y, a, b, c) ) / uvw

def find_center_and_radius_of_sphere(point_1,point_2,point_3,point_4):
    """Finds the centerpoint and the radius of a sphere defined by four points.
    
    Points should be formatted as a list of three coordinates X, Y, and Z.

    Returns the centerpoint and the radius.

    Returns False if the points are coplanar or any three points are colinear.
    """
    is_coplanar = check_if_lines_are_coplanar([point_1,point_2],[point_3,point_4])
    if is_coplanar == True:
        return "Error: Lines are coplanar. (find_center_and_radius_of_sphere)"
    is_colinear_1 = are_these_three_points_colinear([point_1,point_2,point_3])
    is_colinear_2 = are_these_three_points_colinear([point_1,point_2,point_4])
    is_colinear_3 = are_these_three_points_colinear([point_1,point_3,point_4])
    is_colinear_4 = are_these_three_points_colinear([point_2,point_3,point_4])
    if is_colinear_1 or is_colinear_2 or is_colinear_3 or is_colinear_4:
        return "Error: Colinear points detected. (find_center_and_radius_of_sphere)"
    ...
    uvw = 2*(u+v+w)
    if abs(uvw) < 1E-8:
        return "Error: Lines are coplanar. (find_center_and_radius_of_sphere check_2)"
    
    ra = point_1[0]**2+point_1[1]**2+point_1[2]**2
    rb = point_2[0]**2+point_2[1]**2+point_2[2]**2
    rc = point_3[0]**2+point_3[1]**2+point_3[2]**2
    rd = point_4[0]**2+point_4[1]**2+point_4[2]**2

    x = 0
    y = 1
    z = 2

    x0 = get_E_for_sphere(y, z, point_1, point_2, point_3, point_4, ra, rb, rc, rd, uvw)
    y0 = get_E_for_sphere(z, x, point_1, point_2, point_3, point_4, ra, rb, rc, rd, uvw)
    z0 = get_E_for_sphere(x, y, point_1, point_2, point_3, point_4, ra, rb, rc, rd, uvw)
    center = [x0,y0,z0]

    radius = np.sqrt((x1-x0)**2+(y1-y0)**2+(z1-z0)**2)
        ...
        for k in range(len(polyhedron[j])):
            ...
            if type(intersects_edge) == list:
                ...
                if point_on_edge:
                    ...
                    print(f"Random intersection found. Rerunning.")
...
def find_polygon_area(planar_polygon):
    """Calculates the area of a polygon from the coordinate points. The polygon must be planar and convex.
...
def find_intersection_of_plane_and_line_segment(triangle,line_segment):
    ...
    if np.abs(dot_product_decimal(line_unit_vector,plane_equation[0])) >= tolerance:
